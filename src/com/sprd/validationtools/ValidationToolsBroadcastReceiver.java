...
import com.android.adb.InitFileUtils;
import com.android.adb.Custom2ValidationMainActivity;
public class ValidationToolsBroadcastReceiver extends BroadcastReceiver {
	...
	SystemProperties.set("persist.sys.get.RUNIN", parse.getRUNINPhaseCheck());
        ...
        if("83789".equals(host)) {
            i.setClass(context, ValidationToolsMainActivity.class);
            i.putExtra("mode",2);//SMT
            context.startActivity(i);
        }else if ("37".equals(host)) {
            i.setClass(context, ValidationToolsMainActivity.class);
            i.putExtra("mode",0);
            context.startActivity(i);//MMI1
        } else if ("38".equals(host)) {
            i.setClass(context, Custom2ValidationMainActivity.class);
            i.putExtra("mode",1);
            context.startActivity(i);//MMI2
        }else if ("380".equals(host)) {
            i.setClass(context, ValidationToolsMainActivity.class);
            i.putExtra("mode",1);
            context.startActivity(i);//MMI2
