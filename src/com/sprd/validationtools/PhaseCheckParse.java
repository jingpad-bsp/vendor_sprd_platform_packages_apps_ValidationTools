...
//          String strItem = Integer.toBinaryString(item);
    ...
    public String getPhaseCheckState(String name) {
        String result = "no station info";
        Log.d(TAG, "getPhaseCheckState name = " + name);
        try {
            Parcel data = new Parcel();
            Parcel reply = new Parcel();
            binder.transact(TYPE_GET_PHASECHECK, data, reply, 0);
            Log.e(TAG, "getPhaseCheckState transact SUCESS!!");
            int testSign = reply.readInt();
            int item = reply.readInt();
            String stationName = reply.readString().trim();
            Log.d(TAG, "getPhaseCheckState stationName = " + stationName);
            String[] str = stationName.split(Pattern.quote("|"));
//          String strTestSign = Integer.toBinaryString(testSign);
//          String strItem = Integer.toBinaryString(item);
            String strTestSign = ByteUtils.intToBinary32(testSign,32);
            ...
            for (int i = 0; i < str.length; i++) {
                ...
                Log.d(TAG, "getPhaseCheckState str[" + i + "] = " + str[i]);
                if(str[i].equals(name)){
                    Log.e(TAG, "getPhaseCheckState str[i].equals(name)");
                    result=StationTested(charSign[charSign.length-i-1], charItem[charItem.length-i-1]);
                    Log.e(TAG, "getPhaseCheckState result = " + result);
                }
            }
            //result = sb.toString();
            data.recycle();
 ...
 public String getRUNINPhaseCheck() {
        String result = null;
        try{
            Parcel data = new Parcel();
            Parcel reply = new Parcel();
            ...
            char[] charItem = strItem.toCharArray();
            StringBuffer sb = new StringBuffer();
            Log.e(TAG, "qyg---->strTestSign = " + strTestSign + " strItem = " + strItem);
            for(int i = 0; i < str.length; i++) {
		Log.e(TAG, "qyg---->str[i] = " + str[i]);
				if(str[i].equals("RUNIN")){
		Log.e(TAG, "qyg---->RUNIN ");
					result=StationTested(charSign[charSign.length-i-1], charItem[charItem.length-i-1]);
				}
               /* sb.append(str[i]+":"+StationTested(charSign[charSign.length-i-1], charItem[charItem.length-i-1]));
